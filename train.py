...
Single-GPU by design.  There is no DDP path and no multi-process launcher: the
routed block's per-hypernet dispatch skips hypernetworks with no tokens, which is
a data-dependent branch that two ranks can disagree about, and that gather is what
makes the model affordable to run at all.  Restoring multi-GPU means giving that
up first.
"""
import argparse
import contextlib
...
def main():
    ...
    if kind == "cuda":
        # TF32 for fp32 matmuls and a cuDNN autotune pass.  Both are free speed on
        # one GPU: TF32 keeps fp32's exponent range and only trims mantissa bits,
        # and benchmark mode pays a one-off search to pick the best kernels for
        # shapes that never change after the first step.
    ...
    print(f"device: {kind} {device}  amp {amp}", flush=True)
    ...
    decay = [q for n, q in model.named_parameters() if not nodec(n, q)]
    nodecay = [q for n, q in model.named_parameters() if nodec(n, q)]
    ...
    it = batches(data["train"], a.batch, device)
    hist, t0 = [], time.time()
    model.train()
    ...
    # and re-fit the step count to the budget.
    ...
    while True:
        ...
        for i in range(a.accum):
            ...
            scaler.scale((loss + ewc.penalty(model)) / a.accum).backward()
        ...
        if a.max_hours and step == start + PROBE and t_probe:
            ...
            fits = int((a.max_hours * 3600 - (time.time() - t0)) / per)
            budget = max(a.warmup + 200, min(a.steps, fits + step))
            tok = budget * a.batch * a.accum * a.seq_len
        ...
        if step % a.log_every == 0:
            lm /= a.accum
            tok_s = (step - start + 1) * a.batch * a.accum * a.seq_len / (time.time() - t0)
            ...
            print(f"step {step:6d}  loss {lm:.4f}  ppl {math.exp(min(20, lm)):9.2f}  "
                  ...
                  f"[{int(d3.min())}-{int(d3.max())}]  {tok_s:.0f} tok/s", flush=True)
            st = getattr(model, "aux_stats", None)
            if st:
    ...
    if not a.no_ewc_consolidate:
        ewc.consolidate(model, batches(data["train"], a.batch, device, seed=1))
